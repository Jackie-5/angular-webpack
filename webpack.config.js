...
// const cortexJson = JSON.parse(fs.readFileSync('cortex.json'));
...
const copyFile = ['route-tpl', 'images', 'external', 'js/public'];

ENV !== 'build' && copyFile.push('midas');

const recursive = (file, src)=> {
    fs.readdirSync(path.join(file, src)).forEach((item)=> {
        if (item !== 'lib' && item !== 'public') {
            if (!!~item.indexOf('.js')) {
...
module.exports = (()=> {
    ...
    // 添加输出目录,如果是cortex的那么输出为kepler-pc-cortex的目录
    if (ENV === 'build') {
        exec('rm -rf ./build');
